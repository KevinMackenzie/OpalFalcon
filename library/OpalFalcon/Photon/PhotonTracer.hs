...
shootPhoton minBounces scene photon =
  let shoot depth ph@(EPhoton epr _) =
        case probeCollection (sceneObjects scene) epr of
          Nothing -> return ([], [])
          Just (obj, h) -> bounce depth ph obj h
      bounce depth (EPhoton (Ray _ prDir) pCol) obj h@(MkHit {hitPos = hPos}) =
        let iDir = negateVec prDir
            next (EPhoton pRay refl) =
              ...
              let oCol' = pCol |*| refl
                  ...
                  res = shoot (depth + 1) (EPhoton (advanceRay pRay ptEpsilon) oCol)
                  mO = vecCompSum oCol
                  mP = vecCompSum pCol
               in res -- if (abs $ mO - mP) > 0.0001*mP then trace ((show $ mP) ++ " --> " ++ (show $ mO)) res else res
         in do
              result <- transmitPhoton (objHitMat obj h) iDir
              case result of
                PhotonAbsorb -> return ([], [])
